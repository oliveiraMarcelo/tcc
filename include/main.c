...

{
	...
	/* Razer */
	gcmd_line.makeMHF = 0;
	gcmd_line.makePURE = 0;
	gcmd_line.makePREFS = 0;
	gcmd_line.makeISCAS = 0;
...

{
	...
	printf(
			...
			gtempl_time + greach_time + grelev_time + gconn_time + gbuild_time
					+ gcnf_time);
}
...

{
	...
	/* Razer e Bruno */
	printf("\n-B <0, 1 or 2>   to generate PBS formulas\n");
	/* Razer */
	printf("\n-h <0 or 1>   to generate MHF formulas\n");
	printf("\n-u <0 or 1>   to generate pure literals clauses to MHF formulas\n");
	printf("\n-e <0 or 1>   to generate prefs file\n\n");
	...
	while (--argc && ++argv) {
		...
		default:
			if (--argc && ++argv) {
				...
				/* Razer e Bruno */
				case 'B':
					sscanf(*argv, "%d", &gcmd_line.makePBS);
					break;
				/* Razer */
				case 'n':
					sscanf(*argv, "%d", &gcmd_line.makeISCAS);
					break;
					/* Razer */
				case 'h':
					sscanf(*argv, "%d", &gcmd_line.makeMHF);
					break;
				/* Razer */
				case 'u':
					sscanf(*argv, "%d", &gcmd_line.makePURE);
					break;
				/* Razer */
				case 'e':
					sscanf(*argv, "%d", &gcmd_line.makePREFS);
					break;
				case 'S':
...
int create_solution() {
	...
	if ((fp = fopen(gcmd_line.input_solution, "r")) == NULL ) {
		printf("\n\nEXIT: can't open resulting input solution file: %s.\n\n", gcmd_line.input_solution);
		exit(1);
	}
...
void prune(PairVals* varList, int num_vars) { /* varList has been sorted on plan-levels */
	int i = 0, j = 0, k = 0, m = 0, currentValue = -1, n = 0, p = 0,
			breakAgain = 0;
	int ok = 1, mark = 0, ft;
	...
	while (++i < num_vars && varList[i].first == 0) {
		...
		for (n = 0; n < gop_conn[lop_to_code[varList[i].second]].num_P; ++n) {
			for (p = 0; p <= currentValue; ++p) {
				if (gop_conn[lop_to_code[varList[i].second]].P[n]
						== conditions[p]) {
					++k;
					break;
				}
	...
	while (++i < num_vars && varList[i].first == 0) {
		...
		for (n = 0; n < gop_conn[lop_to_code[varList[i].second]].num_A; ++n) {
			for (m = 0; m <= currentValue; ++m) {
				if (gop_conn[lop_to_code[varList[i].second]].A[n]
						== conditions[m])
					break; /* nothing new */
			}
			if (m > currentValue) {
				...
				conditions[++currentValue] =
						gop_conn[lop_to_code[varList[i].second]].A[n];
			}
		}
		if (k == 0)
			varList[i].keep_state = -1;
	...
	while (++i < num_vars - 1) {
		...
		while (j < num_vars && varList[mark].first == varList[j].first) {
			...
			k = 0;
			for (n = 0; n < gop_conn[lop_to_code[varList[j].second]].num_A;
					++n) {
				for (m = 0; m <= currentValue; ++m) {
					if (gop_conn[lop_to_code[varList[j].second]].A[n]
							== conditions[m])
						break;
				}
				if (m > currentValue) {
					k = 1;
					conditions[++currentValue] =
							gop_conn[lop_to_code[varList[j].second]].A[n];
				}
			}
			if (k == 1) { /* keep */
				for (n = 0; n < gop_conn[lop_to_code[varList[j].second]].num_D;
						++n) {
					for (m = 0; m <= currentValue; ++m) {
						if (gop_conn[lop_to_code[varList[j].second]].D[n]
								== conditions[m]) {
	...
	for (i = num_vars - 1; i >= 0; --i) {
		...
		for (m = 0; m <= currentValue; ++m) {
			for (n = 0; n < gop_conn[lop_to_code[varList[i].second]].num_A;
					++n) {
				if (gop_conn[lop_to_code[varList[i].second]].A[n]
						== conditions[m]) {
					/* match found */
	...
	for (i = num_vars - 1; i >= 0;) {
		...
		while (i > j) {
			if (varList[i].keep_state > 1) { /* more preconditions to add */
				for (k = 0; k < gop_conn[lop_to_code[varList[i].second]].num_P;
						++k) {
					for (p = 0; p <= currentValue; ++p) {
						if (gop_conn[lop_to_code[varList[i].second]].P[k]
								== conditions[p]) {
					...
					if (ok)
						conditions[++currentValue] =
								gop_conn[lop_to_code[varList[i].second]].P[k];
					ok = 1;
				}
			}
			--i;
		...
		for (k = j + 1; k <= i && (k > -1); ++k) {
			...
			breakAgain = 0;
			for (m = 0; m <= currentValue; ++m) {
				for (n = 0; n < gop_conn[lop_to_code[varList[k].second]].num_A;
						++n) {
					if (gop_conn[lop_to_code[varList[k].second]].A[n]
							== conditions[m]) {
						/* match found */
...
int get_solution(FILE* fp, FILE* solnFile) {
	...
	while (beg <= markEnd) {
		...
		case '\n':
			...
			if (!done) {
				toConvert[++idx] = '\0';
				/* Razer - aqui a solucao eh lida */
				value = atol(toConvert);

				/* Razer e Bruno - a solucao deve vir sem precisar desconverter (negar) as acoes) */

				/* Razer - Fazer um if aqui para desconverter de MHF */
				if (gcmd_line.makeMHF == 1 && (gcmd_line.cnfout == 1 || gcmd_line.cnfout == 4)) {
					value = (-1) * value;
				}
				...
				if (value > 0
						&& (gop_conn[lop_to_code[value]].noop_for == -1)) {
		...
		default:
			if (++idx > 19) {
				printf("\n\nEXIT: wow, that's a big number - too big - %c\n\n", c);
				fclose(fp);
				fclose(solnFile);
				exit(1);
			}
	...
	for (j = 0; j < counter; ++j) {
		if (toOrder[j].keep_state > 1) {
			...
			if (goperators[gop_conn[lop_to_code[value]].op]->name)
				fprintf(solnFile, "%s",
						goperators[gop_conn[lop_to_code[value]].op]->name);
			for (i = 0;
					i < goperators[gop_conn[lop_to_code[value]].op]->num_vars;
					i++)
				fprintf(solnFile, " %s",
						gconstants[gop_conn[lop_to_code[value]].inst_table[i]]);
			fprintf(solnFile, ") [1]\n");
		} else {
			/*printf("Pruned:  ");
			value = toOrder[j].second;
